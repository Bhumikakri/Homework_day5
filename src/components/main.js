...
// -------------------generate Paragraph------------
    const generatePara = ()=>{
        ...
        if(val<1){
            settempArr([]);
            alert("😉 select a number to get your article.");
        }
        else if(val>paraArray.length){
            settempArr([]);
            setval(0);
            alert("ohhno! 🙄 u choose little little number please");
        }
        else{
            settempArr(
                paraArray.map((value,indx)=>{
                    if(indx<=val-1){
                        return value
                    }
                })
    ...
    return(
        ...
        <input type="number" onChange={getnumber} value={val}></input>
        <button className="GenerBtn" onClick={generatePara}>GENERATE</button>
